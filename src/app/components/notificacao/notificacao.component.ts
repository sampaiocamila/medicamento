...
export class NotificacaoComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    this.settings = of({
      show_action: {
        ...
        consulta_placeholder: 'Digite aqui algo relacionado ao Paciente'
      }
    });
    const _lresult = [];
    this.service.datas.filter(d => d.administracao && !d.notificacao).forEach((data) => {
    ...
    return [
      {
        ...
        favorite: '0', name: 'id_dispensacao', value: data.id_dispensacao,
      ...
      },
      {
        ...
        favorite: null, name: 'dosage', value: `Via de notificação: ${data.dosage.route}, Intervalo: ${data.dosage.intervalo}, Duração: ${data.dosage.duracao} `,
        validators: [], disabled: true, hideRequired: true
      },
      {
